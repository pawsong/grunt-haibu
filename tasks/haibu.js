...
module.exports = function(grunt) {
    ...
    grunt.registerMultiTask('haibu', 'Deploy to haibu server.', function() {
        ...
        var promptSchema,
            detectedInQueue = false;

        var done = this.async();

        for(i = 0; i < grunt.task._queue.length; i++) {
            var task = grunt.task._queue[i].task;

            if(task && task.name === this.name) {

                detectedInQueue = true;
                break;
            }
        }

        if (detectedInQueue && !multiDeployAllowed && options.askWhenMultiDeploy) {
            grunt.log
                .writeln("You are trying to deploy app to multiple server URLs.")
                .writeln("Is this what you really mean to do?");
            ...
            prompt.get(promptSchema, function(err, result) {
                ...
                if(result.allowed === 'yes') {
                    multiDeployAllowed = true;
                    main();
                } else {
